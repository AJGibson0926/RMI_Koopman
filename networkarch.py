...
def encoder_apply(x, weights, biases, act_type, shifts_middle, name='E', num_encoder_weights=1):
    ...
    In other words, compute Phi(x): the Koopman states (vector of Koopman eigenfunctions evaluated at dynamical state x)
    ...
    for j in np.arange(num_shifts_middle + 1):
        ...
        y.append(encoder_apply_one_shift(x_shift, weights, biases, act_type, name, num_encoder_weights))
...
def encoder_apply_one_shift(prev_layer, weights, biases, act_type, name='E', num_encoder_weights=1):
    ...
    final = tf.matmul(prev_layer, weights['W%s%d' % (name, num_encoder_weights)]) + biases['b%s%d' % (name, num_encoder_weights)]
    
    return final


#def encoder_grad(prev_layer, weights, biases, act_type, name='E', num_encoder_weights=1):
...
def decoder_apply(prev_layer, weights, biases, act_type, num_decoder_weights):
    ...
    In other words, compute Phi^{-1}(y): the dynamical state x corresponding to a given vector of Koopman states. NOTE: If the Koopman model is deficient, this may not be a well-defined inverse. Make sure dimension of model is sufficient!
...
def omega_net_apply(params, ycoords, weights, biases):
    ...
    for oidx, o in enumerate(fixed_omegas):
        om = tf.constant([[np.imag(o), np.real(o)]], dtype=tf.float64)
        omegas.append(om)
