...
def try_net(data_val, params):
    ...
    for f in range(params['data_train_len'] * params['num_passes_per_file']):
        ...
        if (params['data_train_len'] > 1) or (f == 0):
            ...
            data_train = np.loadtxt(('./data/TVT/%s/%s_train%d_x.csv' % (params['train_data'], params['data_name'], file_num)), delimiter=',',
                                    dtype=np.float64)
        ...
        for step in range(params['num_steps_per_batch'] * num_batches):
            ...
            if step % 20 == 0:
                ...
                if val_error < (best_error - best_error * (10 ** (-5))):
                    ...
                    print("New best: train error %f and val error %f (with reg. train err %f and reg. val err %f)" % (
                        train_error, best_error, reg_train_err, reg_val_err))
