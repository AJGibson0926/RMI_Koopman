...
def omega_net_apply(params, ycoords, W, b, num_weights, act_type='relu'):
    omegas = []
    if 'fixed_omegas' in params.keys():
        fixed_omegas = params['fixed_omegas']
    else:
        fixed_omegas = []
    ...
    for oidx, o in enumerate(fixed_omegas):
        om = np.vstack([[np.imag(o), np.real(o)]]*ycoords.shape[0])
        omegas.append(om)
...
def decode_Koopman_TS(koop_states, nn):
    dyn_states = []
    prms, W, b = grab_nn(nn)
    num_weights = prms['num_decoder_weights']

    for j in np.arange(koop_states.shape[0]):
        dyn_state = decoder_apply(koop_states[j,:], W, b, 'D', num_weights)
        dyn_states.append(dyn_state)

    dyn_states = np.array(np.stack(dyn_states))        
    return dyn_states
...
def encode_Koopman_TS(dyn_states, nn):
    koop_states = []
    prms, W, b = grab_nn(nn)
    num_weights = prms['num_encoder_weights']

    for j in np.arange(dyn_states.shape[0]):
        koop_state = encoder_apply(dyn_states[j,:], W, b, 'E', num_weights)
        koop_states.append(koop_state)

    koop_states = np.array(np.stack(koop_states))        
    return koop_states


def generate_Koopman_TS(x, nn, num_steps):
    yk_hist = []
    omega_hist = []
    prms, W, b = grab_nn(nn)
    
    if 'fixed_omegas' in prms.keys():
        num_fixed_omegas = len(prms['fixed_omegas'])
    else:
        num_fixed_omegas = 0
        
    delta_t = prms['delta_t']
    num_real = prms['num_real']
    num_complex_pairs = prms['num_complex_pairs']
    num_weights = prms['num_encoder_weights']
    num_omega_weights = prms['num_omega_weights']

    for j in np.arange(num_steps):
        if j==0:
            ypred = encoder_apply(x, W, b, 'E', num_weights)

        omegas = omega_net_apply(prms, ypred, W, b, num_omega_weights)
        omegas_add = np.array(np.hstack(omegas))
        omega_hist.append(omegas_add)

        if j>0:
            ypred = varying_multiply(ypred, omegas, delta_t, num_real, num_complex_pairs + num_fixed_omegas)

        yk_hist.append(ypred)
    
    yk_hist = np.array(np.stack(yk_hist))
    omega_hist = np.stack(omega_hist)
    omega_hist = np.reshape(omega_hist[:,0,:],(omega_hist.shape[0],omega_hist.shape[2]))
    
    return yk_hist, omega_hist


def ApplyKoopmanNetOmegas(params, x, W, b, delta_t, num_real, num_complex_pairs, num_encoder_weights, num_omega_weights,
                          num_decoder_weights):
    # print x.shape
    
    if 'fixed_omegas' in params.keys():
        num_fixed_omegas = len(params['fixed_omegas'])
    else:
        num_fixed_omegas = 0

    yk = encoder_apply(x, W, b, 'E', num_encoder_weights)
    omegas = omega_net_apply(params, yk, W, b, num_omega_weights)

    ykplus1 = varying_multiply(yk, omegas, delta_t, num_real, num_complex_pairs + num_fixed_omegas)
    omegas = omega_net_apply(params, ykplus1, W, b, num_omega_weights)
    ykplus2 = varying_multiply(ykplus1, omegas, delta_t, num_real, num_complex_pairs + num_fixed_omegas)
    omegas = omega_net_apply(params, ykplus2, W, b, num_omega_weights)
    ykplus3 = varying_multiply(ykplus2, omegas, delta_t, num_real, num_complex_pairs + num_fixed_omegas)
...
def PredictKoopmanNetOmegas(x, nn, num_steps):
    prms, W, b = grab_nn(nn)
    ...
    # Generate Koopman time series
    yk_hist, omega_hist = generate_Koopman_TS(x, nn, num_steps)

    # Translate back into dynamical states
    prediction = decode_Koopman_TS(yk_hist, nn)
    
    return prediction, yk_hist, omega_hist


def ApplyKoopmanNetOmegasFull(params, x_stacked, W, b, delta_t, num_shifts, num_shifts_middle, num_real,
    ...
    omegas = omega_net_apply(params, encoded_layer, W, b, num_omega_weights)
    advanced_layer = varying_multiply(encoded_layer, omegas, delta_t, num_real, num_complex_pairs + num_fixed_omegas)
    for shift in np.arange(num_shifts):
        y.append(decoder_apply(advanced_layer, W, b, 'D', num_decoder_weights))
        omegas = omega_net_apply(params, advanced_layer, W, b, num_omega_weights)
        advanced_layer = varying_multiply(advanced_layer, omegas, delta_t, num_real, num_complex_pairs + num_fixed_omegas)
...
def define_loss(x, y, g_list, params, W, b):
    ...
    if params['num_shifts_middle'] > 0:
        omegas = omega_net_apply(params, g_list[0], W, b, num_omega_weights)
        next_step = varying_multiply(g_list[0], omegas, params['delta_t'], params['num_real'],
                                     params['num_complex_pairs'] + num_fixed_omegas)
        for j in np.arange(max(params['shifts_middle'])):
            ...
            omegas = omega_net_apply(params, next_step, W, b, num_omega_weights)
