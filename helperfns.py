...
def stack_data(data, num_shifts, len_time):
    ...
    data_tensor = np.zeros([num_traj * new_len_time, num_shifts + 1, n])
    ...
    for j in np.arange(num_shifts + 1):
        for count in np.arange(num_traj):
            ...
            data_tensor[data_tensor_range, j, :] = data[count * len_time + j: count * len_time + j + new_len_time, :]
...
def save_files(nn, sess, csv_path, train_val_error):
    ...
    # assemble weights and biases dictionaries
    Ws = []
    bs = []
    
    Ws.append(nn.encoder.W)
    Ws.append(nn.decoder.W)
    bs.append(nn.encoder.b)
    bs.append(nn.decoder.b)

    for j in np.arange(nn.omeganet.count):
        Ws.append(nn.omeganet.nets[j].W)
        bs.append(nn.omeganet.nets[j].b)
...
def set_defaults(params):
    ...
    if 'fixed_omegas' in params:
        num_fixed_omegas = len(params['fixed_omegas'])
    else:
        num_fixed_omegas = 0
    if params['num_evals'] != (2 * (params['num_complex_pairs'] + num_fixed_omegas) + params['num_real']):
        raise ValueError("Error, num_evals must equal 2*(num_compex_pairs + num_fixed_omegas) + num_real")
